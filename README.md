...
## Components Used
*  Unity based udacity's simulator and backyard environment.
* udacidrone API to communicate with simulator based drone and Actual Px4 FC(Flight Computer) based real drones.
* Mavlink communication protocols (python library provides this functionality). 
* event driven using callbacks registration to implement motion plan (very basic square plan).



## Modes of operation completing a square loop of 20 meters



* An x-y (East-North) of local position simulator footage and plot of the drone trajectory while manually flying the drone

<img src="Output_Images/Manual.gif" width="700" height="300" />
<img src="Output_Images/East_North_Manual_Plot.png" width="700" height="300" />

* An x-y (East-North) of local position simulator footage and plot of the drone trajectory from autonomously flying the drone

<img src="Output_Images/Auto.gif" width="700" height="300" />
<img src="Output_Images/East_North_Autonomous_Plot.png" width="700" height="300" />


## Plotting the  (East-North) of local position
Use plot.py

## Task
The required task is to command the drone to fly a 20 meter box at a 3 meter altitude. You'll fly this path in two ways: first using manual control and then under autonomous control.
...
Alternatively, the drone can be manually started/stopped from a python/ipython shell in the fcnd conda environment downloded:
...
The data is stored as a dictionary of message types. For each message type, there is a list of numpy arrays. For example, to access the longitude and latitude from a `MsgID.LOCAL_POSITION`:
